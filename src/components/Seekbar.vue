...
<script lang="ts" setup>
    const {min, max, modelValue} = defineProps<{
        min?: number,
        max?: number,
        modelValue?: number
    }>()
...
<template>
    <label class="pure-material-slider select-none">
        <input @change="onInputChange" :value="modelValue" type="range" :min="min || 1" :max="max || 100">
    </label>
</template>
